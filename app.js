...
const CONST = {
  WARMUP_FLOOR: 50, MAX_SCORE_REF: 102, MAX_HISTORY_SIGS: 100,
  BYPASS_SCORE: 12.0, MULT_SMOOTH_ALPHA: 0.15,
};

/* Default Twelve Data API key — pre-filled so the app works out of the box.
   Users can still override it with their own key in Pengaturan. */
const DEFAULT_API_KEY = 'bbdd38f627f748409e30077b748e5098';
...
let state = {
  ...
  useAsym:true, useCharFlip:true, useEffAtr:true,
...
function computeEngine(candles, cfg){
  ...
  const structHi = rollingHighest(highs, 20), structLo = rollingLowest(lows, 20);
  ...
  if(hasVolume){
    ...
    for(let i=0;i<n;i++){
      const s=Math.max(0,i-20+1); const w=vols.slice(s,i+1);
  ...
  for(let i=0;i<n;i++){
    ...
    // source == close in this port, so close<source / close>source are always false (matches original default behaviour)
    const charFlipDown = false, charFlipUp = false;
  ...
  for(let i=0;i<n;i++){
    ...
    if(tradeDir!==0 && i>entryBar){
      ...
      const slHit = tradeDir===1?lows[i]<=tradeSl:highs[i]>=tradeSl;
      ...
      if(tp3r || slHit || timeoutHit){
        // equal-thirds simplified realized R model
        let legs=[hit[0]?tradeTpR[0]:-1, hit[1]?tradeTpR[1]:-1, hit[2]?tradeTpR[2]:-1];
  ...
  return {
    ...
    stats:{count:allCount, sumR:allSumR, winRate: allCount? rBuffer.filter(r=>r>0).length/allCount*100 : null,
      avgR: allCount? allSumR/allCount : null, curWinStreak, curLossStreak, maxWinStreak, maxLossStreak},
    last,
  };
}
...
function renderAll(res, candles){
  ...
  if(ot){
    ...
    document.getElementById('tScore').textContent = ot.sig ? ot.sig.score.toFixed(1)+' /102' : '—';
  } else {
  ...
  if(state.notif && state.notifPermission){
    try{
      new Notification(`GUSERA SATS — ${sig.side} ${state.symbol}`, {
        body: `Harga ${fmtPrice(sig.price)} · Skor ${sig.score.toFixed(0)}/102 · SL ${fmtPrice(sig.sl)} · TP1 ${fmtPrice(sig.tp1)}`,
      });
    }catch(e){}
  }
}
